...
dbt Labs рекомендует выполнять эти шаги в локальной среде разработки (например, [dbt Cloud CLI](/docs/cloud/cloud-cli-installation)) вместо dbt Cloud IDE:
...
4. Установите [dbt Cloud CLI](/docs/cloud/cloud-cli-installation) для выполнения команд MetricFlow и определения конфигураций вашей семантической модели.
   - Если вы используете dbt Core, установите [MetricFlow CLI](/docs/build/metricflow-commands) с помощью `python -m pip install "dbt-metricflow[your_adapter_name]"`. Например:
...
3. Для получения подробной информации о синтаксисе SQL обратитесь к [Запрос API для метаданных метрик](/docs/dbt-cloud-apis/sl-jdbc#querying-the-api-for-metric-metadata) для выполнения запросов к метрикам с использованием API.
...
3. Обновите вашу среду в **Настройки аккаунта** -> **Детали проекта** -> **Редактировать конфигурацию семантического слоя**, чтобы указать на вашу производственную среду.
...
- [Почему мы снимаем с поддержки пакет dbt_metrics](/blog/deprecating-dbt-metrics) блог
- [Синтаксис запроса API семантического слоя dbt](/docs/dbt-cloud-apis/sl-jdbc#querying-the-api-for-metric-metadata)
