...
### Подготовка к миграции {#preparing-to-migrate}
...
### Операции, которые нам нужно сопоставить {#operations-we-need-to-map}
...
## Сопоставление INSERT {#map-inserts}
...
### **Заметка о `FROM`** {#a-note-on-from-clauses}
...
### **Последовательные `INSERT` в существующую таблицу можно объединить с помощью `UNION ALL`** {#sequential-inserts-to-an-existing-table-can-be-union-alled-together}
...
## Сопоставление UPDATE {#map-updates}
...
## Сопоставление DELETE {#map-deletes}
...
## Сопоставление MERGE {#map-merges}
...
### Шаг 1: Сопоставьте MERGE как INSERT/UPDATE для начала {#step-1-map-the-merge-like-an-insertupdate-to-start}
...
### Шаг 2: Преобразование в инкрементальную материализацию {#step-2-convert-to-incremental-materialization}
...
## Миграция хранимых процедур {#migrate-stores-procedures}
