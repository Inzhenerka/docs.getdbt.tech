...
## Как настроить это поведение {#how-to-customize-this-behavior}
...
## Примеры сценариев использования {#example-use-cases}
...
### 1. Кастомные схемы без конкатенации target-схемы в production {#1-custom-schemas-without-target-schema-concatenation-in-production}
...
### 2. Статические схемы: добавление идентификаторов разработчиков к таблицам {#2-static-schemas-add-developer-identities-to-tables}
...
### 3. Использование имени ветки в качестве префикса схемы {#3-use-branch-name-as-schema-prefix}
...
### 4. Использование статической схемы для CI {#4-use-a-static-schema-for-ci}
...
## Чего делать не стоит {#what-not-to-do}

В этом разделе описано, каких действий следует избегать при настройке схем и алиасов из‑за возможных проблем.

### Обновление generate_schema_name() так, чтобы всегда использовать кастомную схему {#update-generateschemaname-to-always-use-the-custom-schema}
...
### Создание несуществующих баз данных из dbt {#creating-non-existing-databases-from-dbt}
...
### Определение контекста через переменные окружения, а не через `target.name` {#assuming-context-using-environment-variables-rather-than-targetname}
...
### Всегда требовать указания кастомных схем {#always-enforce-custom-schemas}
