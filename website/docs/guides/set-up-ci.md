...
## Создание новой среды CI {#create-a-new-ci-environment}
...
### 1. Дважды проверьте, что ваша продакшн-среда идентифицирована {#1-double-check-your-production-environment-is-identified}
...
### 2. Создайте новую задачу в среде CI {#2-create-a-new-job-in-the-ci-environment}
...
### 3. Протестируйте ваш процесс {#3-test-your-process}
...
## Применение лучших практик с dbt project evaluator {#enforce-best-practices-with-dbt-project-evaluator}
...
### 1. Установите пакет {#1-install-the-package}
...
### 2. Определите серьезность теста с помощью переменной окружения {#2-define-test-severity-with-an-environment-variable}
...
### 3. Обновите ваши команды CI {#3-update-your-ci-commands}
...
### 4. Примените любые настройки {#4-apply-any-customizations}
...
## Запуск проверок линтинга с SQLFluff {#run-linting-checks-with-sqlfluff}
...
### Ручная настройка SQL linting в CI {#manually-set-up-sql-linting-in-ci}
...
### 1. Создайте YAML файл для определения вашего конвейера {#1-create-a-yaml-file-to-define-your-pipeline}
...
### 2. Зафиксируйте и отправьте ваши изменения, чтобы убедиться, что все работает {#2-commit-and-push-your-changes-to-make-sure-everything-works}
...
## Продвинутое: Создание релизного поезда с дополнительными средами {#advanced-create-a-release-train-with-additional-environments}
...
### Продвинутые предварительные требования {#advanced-prerequisites}

- У вас есть среды **Development**, **CI** и **Production**, как описано в [базовой настройке](/guides/set-up-ci).

### 1. Создайте ветку `release` в вашем git-репозитории {#1-create-a-release-branch-in-your-git-repo}
...
### 2. Обновите вашу среду разработки для использования ветки `qa` {#2-update-your-development-environment-to-use-the-qa-branch}
...
### 3. Создайте новую среду QA {#3-create-a-new-qa-environment}
...
### 4. Создайте новую задачу {#4-create-a-new-job}
...
### Необязательно: также добавьте задачу только для компиляции {#optional-also-add-a-compile-only-job}
...
### 5. Протестируйте ваш процесс {#5-test-your-process}
