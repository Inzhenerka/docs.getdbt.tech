...
### Все базы данных не одинаковы {#all-databases-are-not-the-same}
...
### Что нужно адаптировать? {#what-needs-to-be-adapted}
...
### Клиентская библиотека Python и менеджер соединений {#python-client-library-connection-manager}
...
### Как dbt инкапсулирует и абстрагирует эти различия {#how-dbt-encapsulates-and-abstracts-these-differences}
...
### Архитектура адаптера {#adapter-architecture}
...
### Драйвер / Библиотека соединений {#driver-connection-library}
...
### Поддержка вашего нового адаптера {#maintaining-your-new-adapter}
...
#### Поддержание совместимости кода с dbt Core {#keeping-the-code-compatible-with-dbt-core}
...
#### Версионирование и выпуск вашего адаптера {#versioning-and-releasing-your-adapter}
...
## Создание нового адаптера {#build-a-new-adapter}
...
### Создание каркаса нового адаптера {#scaffolding-a-new-adapter}
...
### Редактирование `setup.py` {#editing-setuppy}
...
#### Методы класса `ConnectionManager` {#connectionmanager-class-methods}
...
##### `open(cls, connection)` {#opencls-connection}
...
##### `get_response(cls, cursor)` {#get_responsecls-cursor}
...
##### `cancel(self, connection)` {#cancelself-connection}
...
##### `exception_handler(self, sql, connection_name='master')` {#exceptionhandlerself-sql-connectionnamemaster}
...
##### `standardize_grants_dict(self, grants_table: agate.Table) -> dict` {#standardizegrantsdictself-grants_table-agatetable-dict}
...
### Редактирование реализации адаптера {#editing-the-adapter-implementation}
...
### Редактирование логики SQL {#editing-sql-logic}
...
#### Обязательные макросы {#required-macros}
...
#### Диспетчеризация адаптера {#adapter-dispatch}
...
#### Переопределение методов адаптера {#overriding-adapter-methods}
...
#### Макросы разрешений {#grants-macros}
...
### Флаги изменения поведения {#behavior-change-flags}
...
## Тестирование вашего адаптера {#test-your-adapter}
...
### Предварительные требования к тестированию {#testing-prerequisites}
...
### О тестовой структуре {#about-the-testing-framework}
...
#### Пример: простой тестовый случай {#example-a-simple-test-case}
...
### Тестирование этого адаптера {#testing-this-adapter}
...
### Категории тестов {#categories-of-tests}
...
### Начало работы с запуском базовых тестов {#getting-started-running-basic-tests}
...
### Настройте и сконфигурируйте pytest {#set-up-and-configure-pytest}
...
### Определите тестовые случаи {#define-test-cases}
...
### Изменение тестовых случаев {#modifying-test-cases}
...
### Запуск с несколькими профилями {#running-with-multiple-profiles}
...
## Документирование нового адаптера {#document-a-new-adapter}
...
### Доступность вашего адаптера {#making-your-adapter-available}
...
### Темы и страницы для охвата {#topics-and-pages-to-cover}
...
### Примеры PR для добавления новой документации адаптера {#example-prs-to-add-new-adapter-documentation}
...
## Продвижение нового адаптера {#promote-a-new-adapter}
...
### Кто должен присоединиться к Slack-сообществу dbt? {#who-should-join-the-dbt-community-slack}
...
### Кто может испытывать трудности в сообществе dbt {#who-might-struggle-in-the-dbt-community}
...
### Кто является аудиторией для выпуска адаптера? {#who-is-the-audience-for-an-adapter-release}
...
### Как сообщить о начальном выпуске и последующем контенте {#how-to-message-the-initial-rollout-and-follow-up-content}
...
### Тактически управляйте распространением контента о новых или существующих адаптерах {#tactically-manage-distribution-of-content-about-new-or-existing-adapters}
...
#### Измерение вовлеченности {#measuring-engagement}
...
### Как объявить о новом адаптере {#how-to-announce-a-new-adapter}
...
#### Объявление о новых версиях существующих адаптеров {#announcing-new-release-versions-of-existing-adapters}
...
## Создание доверенного адаптера {#build-a-trusted-adapter}
...
### Что значит быть доверенным {#what-it-means-to-be-trusted}
...
### Полнота функций {#feature-completeness}
...
### Отзывчивость сообщества {#community-responsiveness}
...
### Как добавить адаптер в список доверенных {#how-to-get-an-adapter-on-the-trusted-list}
...
### Получение помощи для моего доверенного адаптера {#getting-help-for-my-trusted-adapter}
