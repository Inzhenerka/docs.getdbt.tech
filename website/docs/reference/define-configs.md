...
- [`freshness`](/reference/resource-configs/freshness) has the same behavior as the `meta` config (a more specific key-value pair replaces a less specific value with the same key).
