...
| `count` and `period` | Specify how often dbt should check for new data. For example, `count: 4, period: hour` means dbt will check every 4 hours.<br /><br /> <VersionBlock firstVersion="1.10">Note that for every `freshness` config, it is required to either set values for both `count` and `period`, or set `freshness: null`.</VersionBlock>|
