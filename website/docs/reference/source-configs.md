...
- **Note**: To disable a source table nested in a property file in a subfolder, you will need to supply the subfolder(s) within the path to that property file, as well as the source name and the table name in the `dbt_project.yml` file.<br /><br /> 
  The following example shows how to disable a source table nested in a property file in a subfolder: 
