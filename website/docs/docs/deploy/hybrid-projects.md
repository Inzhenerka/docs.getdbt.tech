...
- Enable teams that work in dbt Core and dbt Cloud, particularly in dbt Mesh hybrid workflows, so that if an organization runs dbt Core locally or with external orchestration tools (like Airflow, Dagster), they can still collaborate with dbt Cloud.
...
- Your dbt Core project is configured with model access:
