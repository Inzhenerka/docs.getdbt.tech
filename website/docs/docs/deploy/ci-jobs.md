...
## Триггер CI job через API <Lifecycle status="managed,managed_plus" /> {#trigger-a-ci-job-with-the-api}
...
## Семантические проверки в CI  <Lifecycle status="self_service,managed,managed_plus" /> {#semantic-validations-in-ci}
...
### Настройка семантических проверок в вашем CI задании {#set-up-semantic-validations-in-your-ci-job}
