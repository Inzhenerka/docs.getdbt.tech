...
# GraphQL <Lifecycle status="self_service,managed,managed_plus" /> {#graphql}
...
## GraphQL API семантического слоя dbt {#dbt-semantic-layer-graphql-api}
...
## Требования для использования GraphQL API {#requirements-to-use-the-graphql-api}
...
### Вызовы метаданных {#metadata-calls}

#### Получение диалекта платформы данных {#fetch-data-platform-dialect}
...
#### Получение доступных метрик {#fetch-available-metrics}
...
#### Получение доступных измерений для метрик {#fetch-available-dimensions-for-metrics}
...
#### Получение доступных гранулярностей для заданных метрик {#fetch-available-granularities-given-metrics}
...
#### Получение показателей {#fetch-measures}
...
#### Получение сущностей {#fetch-entities}
...
#### Получение сущностей и измерений для группировки метрик {#fetch-entities-and-dimensions-to-group-metrics}
...
#### Параметры типа метрики {#metric-type-parameters}
...
#### Типы измерений {#dimension-types}
...
#### Список сохранённых запросов {#list-saved-queries}
...
#### Вывести сохранённый запрос {#list-a-saved-query}
...
#### Получение результата запроса {#fetch-query-result}
...
### Формат вывода и пагинация {#output-format-and-pagination}
...
### Запуск запроса на Python {#run-a-python-query}
...
### Дополнительные примеры create-запросов {#additional-create-query-examples}
...
#### Псевдоним метрики запроса {#query-metric-alias}
...
#### Запрос с временной гранулярностью {#query-with-a-time-grain}
...
#### Запрос нескольких метрик и нескольких измерений {#query-multiple-metrics-and-multiple-dimensions}
...
#### Запрос по категориальному измерению отдельно {#query-a-categorical-dimension-on-its-own}
...
#### Запрос с фильтром `where` {#query-with-a-where-filter}
...
#### Многошаговые соединения {#multi-hop-joins}
...
```
#### Запрос с сортировкой {#query-with-order}
...
#### Запрос с ограничением (limit) {#query-with-limit}
...
#### Запрос сохранённых запросов {#query-saved-queries}
...
#### Запрос только с компиляцией SQL {#query-with-just-compiling-sql}
...
#### История запросов {#query-records}
