...
## Состояние определения (логическое) vs. примененное состояние узлов dbt {#definition-logical-vs-applied-state-of-dbt-nodes}
...
## Состояния, затронутые типом узла {#affected-states-by-node-type}
...
## Предостережения о обновлениях состояния/метаданных {#caveats-about-statemetadata-updates}
