...
# JDBC API <Lifecycle status="self_service,managed,managed_plus" /> {#jdbc-api}
...
## Использование JDBC API {#using-the-jdbc-api}
...
## Запрос метаданных через API {#querying-the-api-for-metadata}
...
## Запрос значений через API {#querying-the-api-for-values}
...
### Примечание о временных измерениях и `metric_time` {#note-on-time-dimensions-and-metric_time}
...
### Запрос общих измерений {#fetch-metadata-for-metrics}
...
### Запрос с группировкой по времени {#query-common-dimensions}
...
### Запрос с временной гранулярностью {#query-grouped-by-time}
...
### Группировка по категориальному измерению {#query-with-a-time-grain}
...
### Запрос только измерения {#group-by-categorical-dimension}
...
### Запрос по всем измерениям {#query-only-a-dimension}
...
### Запрос с фильтрами `where` {#query-by-all-dimensions}
...
### Запрос с ограничением {#query-with-where-filters}
...
### Примеры запросов с `order by` {#query-with-a-limit}
...
### Запрос с ключевым словом compile {#query-with-order-by-examples}
...
### Запрос сохраненного запроса {#query-with-compile-keyword}
...
### Алиасы метрик в запросах {#query-a-saved-query}
...
### Многошаговые (multi-hop) соединения {#query-metric-alias}
