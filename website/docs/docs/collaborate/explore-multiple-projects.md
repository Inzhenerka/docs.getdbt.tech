...
When viewing a project's lineage in dbt Explorer, only _directly_ referenced public models appear. [Indirect dependencies are not displayed](/faqs/Project_ref/indirectly-referenced-upstream-model). If a referenced model in your project depends on another upstream public model, the second-level model won't appear in Explorer, however it will appear in the [dbt Cloud IDE](/docs/cloud/dbt-cloud-ide/develop-in-the-cloud) lineage view.
:::
