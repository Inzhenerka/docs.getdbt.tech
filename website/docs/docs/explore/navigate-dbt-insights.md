...
# Навигация по интерфейсу dbt Insights <Lifecycle status="managed,managed_plus" /> {#navigate-the-dbt-insights-interface}
...
### Меню Query console {#query-console-menu}
...
## Query Builder <Lifecycle status="beta" /> {#query-builder}
...
## Панель Query output {#query-output-panel}
...
## Боковое меню Query console {#query-console-sidebar-menu}
...
### История запросов (Query history) {#query-history}
