...
- (Optional) To get [Language Server Protocol (LSP) features](/docs/explore/navigate-dbt-insights#lsp-features-in-dbt-insights) in <Constant name="query_page" /> and have your compilation run on the <Constant name="fusion_engine" />, set your development environment to use the **Latest Fusion** dbt version.
