...
All runs available in <Constant name="cloud"/> will be migrated with your account. This includes metadata about the run like its status, when it began executing, and the duration. The individual steps associated with a run will not be migrated with your account. Therefore, the <Constant name="dbt"/> commands executed during a run, along with their logs and artifact files, will not be available in <Constant name="cloud"/> after your migration.
