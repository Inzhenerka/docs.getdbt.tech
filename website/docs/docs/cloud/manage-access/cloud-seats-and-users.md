...
- **Read-Only** &mdash; Intended to view the [artifacts](/docs/deploy/artifacts) created in a dbt Cloud account. Read-Only users can receive job notifications but not configure them. _Read-only licenced users do not inherit rights from any permisson sets_. Every read-only access across the account regardless of the group permissions assigned.
