...
# Настройка SCIM <Lifecycle status="managed, managed_plus" /> {#set-up-scim}
...
### Формат имени пользователя для SCIM {#scim-username-format}
...
### Существующие интеграции Okta {#existing-okta-integrations}
...
## Конфигурация SCIM для Entra ID {#scim-configuration-for-entra-id}
...
### Настройка Entra ID {#set-up-entra-id}
...
### Маппинг атрибутов {#attribute-mapping}
...
## Назначение пользователей SCIM-приложению {#assign-users-to-scim-app}
...
## Управление лицензиями пользователей с помощью SCIM {#manage-user-licenses-with-scim}
...
### Добавление атрибута типа лицензии для Okta {#add-license-type-attribute-for-okta}
