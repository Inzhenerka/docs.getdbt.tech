...
- (Recommended) Import and manage existing <Constant name="cloud" /> groups and users using Okta's **Import Groups** and **Import Users** features. Update the groups in your IdP with the same naming convention used for <Constant name="cloud" /> groups. New users, groups, and existing profile changes will be automatically imported into <Constant name="cloud" />.
