...
- **Create <Constant name="cloud" /> User**: This will create a new entry in the <Constant name="cloud" /> database for the new user. This user record contains the user's email address, first and last name, and any IdP attributes (e.g. groups) passed along from the Identity Provider. dbt Cloud will send a verification email, and the user needs to follow the steps [here](/docs/cloud/manage-access/invite-users#user-experience).
