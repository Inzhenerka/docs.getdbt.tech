...
# О доступе пользователей {#about-user-access}
...
### Пароли пользователей {#user-passwords}
...
### Создание новых групп <Lifecycle status="managed,managed_plus" /> {#create-new-groups}
...
### Доступ и разрешения группы {#group-access-and-permissions}
...
#### Права на запись в окружения (Environment write access) {#environment-write-access}
...
### Сопоставления SSO <Lifecycle status="managed,managed_plus" /> {#sso-mappings}
...
## Управление доступом на основе ролей <Lifecycle status="managed,managed_plus" /> {#role-based-access-control}
...
### Управление лицензиями SCIM {#scim-license-management}
