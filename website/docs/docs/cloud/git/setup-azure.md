...
В результате MFA должна быть явно отключена в панели администрирования Office 365 или Microsoft Entra ID для сервисного пользователя. Просто "отключение" не будет достаточным, так как dbt Cloud будет предложено настроить MFA вместо того, чтобы использовать учетные данные по назначению.
