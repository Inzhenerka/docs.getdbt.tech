...
- <Constant name="cloud" /> developers on the [Enterprise or Enterprise Plus plan](https://www.getdbt.com/pricing/) must each connect their GitHub profiles to <Constant name="cloud" />. This is because the <Constant name="cloud_ide" /> verifies every developer's read / write access for the dbt repo. 
