...
- **[Connection setup](#connection-setup):** Set up the credentials used to access the data warehouse information. You can use the connection that has your main account identifier (name or id). Only one unique [connection](/docs/cloud/connect-data-platform/about-connections#connection-management) per warehouse needs to have the credentials configured. If you have two or more connections that reference the same account identifier, you only need to add platform metadata credentials to one of them.
