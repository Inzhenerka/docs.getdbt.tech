...
After setting up your local server, you can now connect the server to Cursor. Log in to Cursor and follow the steps depending on how you're hosting `dbt-mcp`. We recommend using the linked templates rather than the Cursor MCP-hosted template on their integrations page. 
