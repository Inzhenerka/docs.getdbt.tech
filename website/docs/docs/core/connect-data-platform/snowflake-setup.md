...
:::note Limitation
Using a private key through an [environment variable](/docs/build/environment-variables) (for example, `{{ env_var('PRIVATE_KEY') }}`) for Snowflake key pair authentication is currently not supported. 
:::
