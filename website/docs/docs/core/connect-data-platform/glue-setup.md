...
### Настройка AWS‑профиля для Glue Interactive Session {#configuring-your-aws-profile-for-glue-interactive-session}
...
### Настройка локального окружения {#configuration-of-the-local-environment}
...
### Настройка таблиц {#configuring-tables}
...
#### Пример конфигурации профиля {#profile-config-example}
```yaml
test_project:
  target: dev
  outputs:
...
#### Пример исходного кода {#source-code-example}
```sql
{{ config(
    materialized='incremental',
...
#### Пример конфигурации профиля {#profile-config-example-1}
```yaml
test_project:
  target: dev
  outputs:
...
#### Пример исходного кода {#source-code-example-1}
```sql
{{ config(
    materialized='incremental',
    incremental_strategy='merge',
...
#### Пример конфигурации профиля {#profile-config-example-2}
```yaml
test_project:
  target: dev
  outputs:
...
#### Пример исходного кода {#source-code-example-2}
```sql
{{ config(
    materialized='incremental',
    incremental_strategy='merge',
...
group by 1
```
#### Пример исходного кода Iceberg Snapshot {#iceberg-snapshot-source-code-example}
...
## Мониторинг Glue Interactive Session {#monitoring-your-glue-interactive-session}
...
#### Пример конфигурации профиля {#profile-config-example-3}
```yaml
test_project:
  target: dev
  outputs:
...
## Включение AWS Glue Auto Scaling {#enabling-aws-glue-auto-scaling}
...
#### Пример конфигурации профиля {#profile-config-example-4}
```yaml
test_project:
  target: dev
  outputs:
...
## Доступ к Glue catalog в другом AWS account {#access-glue-catalog-in-another-aws-account}
...
#### Пример конфигурации профиля {#profile-config-example-5}
```yaml
test_project:
  target: dev
  outputsAccountB:
...
## Интеграция с AWS Lake Formation {#aws-lakeformation-integration}
...
#### На уровне модели {#at-model-level}
...
#### На уровне dbt-project {#at-dbt-project-level}
