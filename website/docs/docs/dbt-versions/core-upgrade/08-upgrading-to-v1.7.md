...
- **[Set `access` as a config](/reference/resource-configs/access):** You can now set a model's `access` within SQL file configs in the model's file or in the `dbt_project.yml` for an entire subfolder at once.
