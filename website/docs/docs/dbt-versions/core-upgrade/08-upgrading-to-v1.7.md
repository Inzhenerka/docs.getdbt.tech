...
- **[Обнаружение критических (breaking) изменений](/reference/resource-properties/versions#detecting-breaking-changes) для моделей с включёнными контрактами:** Когда dbt обнаруживает критическое изменение модели с принудительно применяемым контрактом во время сравнения состояния (state comparison), он теперь будет выбрасывать ошибку для версионируемых моделей и предупреждение — для моделей без версий.
- **[Задание `access` как конфигурации](/reference/resource-configs/access):** Теперь вы можете задавать `access` модели внутри config-блоков в файле модели или в `dbt_project.yml` сразу для всей подпапки.
- **[Алиасинг типов для контрактов моделей](/reference/resource-configs/contract):** dbt будет использовать встроенный в каждый адаптер алиасинг типов для пользовательских типов данных. Это означает, что теперь вы всегда можете писать `string`, а dbt сам преобразует его в `text` для Postgres/Redshift. Эта возможность включена по умолчанию, но при необходимости от неё можно отказаться.
- **[Предупреждение для числовых типов](/reference/resource-configs/contract):** Из‑за возможных проблем при использовании `numeric` в контрактах моделей без учёта значений по умолчанию (например, `numeric(38,0)`, которые могут округлять дробные значения), dbt теперь будет выдавать предупреждение, если обнаружит числовой тип без явно заданной точности и масштаба.
