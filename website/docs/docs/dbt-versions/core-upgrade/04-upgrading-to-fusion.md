...
In older versions of dbt Core, it was possible to create scenarios with duplicate [docs blocks](/docs/build/documentation#using-docs-blocks). For example, you can have two packages with identical docs blocks referenced by an unqualified name in your dbt project. In this case dbt Core would use whichever docs block is referenced without any warnings are errors. 

Fusion adds stricter evaluation of docs blocks names to prevent such ambiguity. It will present an error if it detects duplicate docs blocks names:
