...
- **Enhancement**: Improved stability in the [Python SDK](/docs/dbt-cloud-apis/sl-python) by increasing the default `execute_timeout` in GraphQL from 10s to 30s to reduce test flakiness (inconsistent results), particularly for the "execute query sync" endpoint. 
