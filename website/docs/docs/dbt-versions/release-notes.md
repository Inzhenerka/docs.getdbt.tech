...
- **Enhancement**:  The [Python SDK](/docs/dbt-cloud-apis/sl-python) added a new timeout parameter to Semantic Layer client and to the underlying GraphQL clients to specify timeouts. Set timeout number or `total_timeout` in the global `TimeoutOptions` to control connect, execute and close timeouts granularly. `ExponentialBackoff.timeout_ms` is now deprecated.
