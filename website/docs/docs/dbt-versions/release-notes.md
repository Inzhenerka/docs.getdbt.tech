...
- **Enhancement**: The [Python SDK](/docs/dbt-cloud-apis/sl-python) has improved stability by increasing the default `execute_timeout` in GraphQL from 10 seconds to 30 seconds to reduce test flakiness (inconsistent results). This is particularly useful for the "execute query sync" endpoint. 
