...
- <Expandable alt_header='Обновления CI'>
    ...
    И, кроме того, теперь у нас есть более эффективные сравнения состояния при CI‑проверках: вы больше никогда не будете тратить сборку или тесты на код, который не изменялся. Теперь мы более эффективно вычисляем различия между кодом в pull request (PR) в <Constant name="git" /> и тем, что запущено в продакшене, благодаря внедрению механизма deferral к окружению, а не к отдельному job. Подробнее об этом читайте в разделе [Continuous integration in <Constant name="cloud" />](/docs/deploy/continuous-integration).
    ...
    Если вы использовали [Create Job](/dbt-cloud/api-v2#/operations/Create%20Job) API-эндпоинт, но не установили `"triggers":triggers.git_provider_webhook`, задание было неверно классифицировано как задание развертывания, и вам нужно будет воссоздать его, как описано в [Запуск CI задания с помощью API](/docs/deploy/ci-jobs#trigger-a-ci-job-with-the-api).
        ...
        Если вы использовали интерфейс <Constant name="cloud" />, но не включили опцию **Run on Pull Requests**, которая находится на вкладке **Continuous Integration** (CI), задание было ошибочно классифицировано как deploy‑задание. В этом случае его необходимо пересоздать, как описано в разделе [Set up CI jobs](/docs/deploy/ci-jobs#set-up-ci-jobs).

        Чтобы проверить тип задания, откройте список CI‑заданий в <Constant name="cloud" /> в разделе [Run History](/docs/deploy/run-visibility#run-history) и посмотрите, есть ли под именем задания тег **CI Job**. Если этого тега нет, значит задание было классифицировано неверно, и его нужно пересоздать.
    ...
    **Обновление CI фазы 3 &mdash; Обновление: Улучшенное автоматическое удаление временных схем** 
    ...
    - В вашем проекте переопределён макрос [generate_schema_name](/docs/build/custom-schemas), но в нём отсутствует обязательный префикс `dbt_cloud_pr_`. Подробности см. в разделе [Troubleshooting](/docs/deploy/ci-jobs#troubleshooting).
    - Вы используете [ненативную интеграцию с <Constant name="git" />](/docs/deploy/ci-jobs#trigger-a-ci-job-with-the-api). Это связано с тем, что автоматическое удаление зависит от входящих вебхуков от провайдеров <Constant name="git" />, которые доступны только при использовании нативных интеграций.
...
- <Expandable alt_header='Обновления документации продукта'>
    ...
    * Виджет обновлений продукта **What’s new?** снова доступен в интерфейсе dbt Cloud! Команда Docs начнёт регулярно обновлять контент, чтобы информировать вас о новых возможностях.
    * Мы запустили обновлённые [документы Semantic Layer (beta)](/docs/use-dbt-semantic-layer/dbt-sl), которые знакомят пользователей с новым API, новым руководством по настройке MetricFlow и нового Semantic Layer, а также переработали раздел **Use the dbt Semantic Layer**.
    * Обновили документацию по [Admin API v2 и v3](/docs/dbt-cloud-apis/admin-cloud-api), чтобы помочь вам разобраться в различиях между версиями и понять, какая из них содержит нужные вам эндпоинты.
    * Чтобы улучшить навигацию, команда документации внесла изменения в боковое меню [deploy dbt](/docs/deploy/deployments). Мы добавили карточки и лучше синхронизировали структуру с интерфейсом dbt Cloud и реальными сценариями использования.
    * Объявили устаревшими legacy-схемы заданий (job schemas) в [Discovery API](/docs/dbt-cloud-apis/discovery-api).
    * Добавили страницу с описанием [экспериментальных и beta-функций](/docs/dbt-versions/experimental-features) в dbt Cloud и тем, что важно о них знать.
    * Добавили раздел, представляющий новую beta-функцию [**Extended Attributes**](/docs/dbt-cloud-environments#extended-attributes-beta), которая позволяет задавать гибкий фрагмент `profiles.yml` в настройках окружения dbt Cloud.

    ## 🎯 Проекты Core 

    * Мы выпустили [dbt 1.6](/docs/dbt-versions/core-upgrade/Older%20versions/upgrading-to-v1.6)! Также добавили документацию для новых команд `dbt retry` и `dbt clone`.

    ## New 📚 Guides, ✏️ blog posts, and FAQs

    * Посмотрите, как участники сообщества используют dbt, в разделе [Community spotlight](/community/spotlight).
    * Среди опубликованных этим летом постов в блоге: [Optimizing Materialized Views with dbt](/blog/announcing-materialized-views), [Data Vault 2.0 with dbt Cloud](/blog/data-vault-with-dbt-cloud) и [Create dbt Documentation and Tests 10x faster with ChatGPT](/blog/create-dbt-documentation-10x-faster-with-ChatGPT).
    - У нас также появились два новых руководства с best practices: [How we build our metrics](/best-practices/how-we-build-our-metrics/semantic-layer-1-intro) и [Set up Continuous Integration](/guides/set-up-ci).

  </Expandable>

- <Expandable alt_header='Removing prerelease versions'>

    Ранее, когда dbt Labs выпускали новую [версию](/docs/dbt-versions/core#how-dbt-core-uses-semantic-versioning) в dbt Cloud, более старая patch-_prerelease_-версия и _latest_-версия одновременно оставались доступными в выпадающем списке в **Environment settings**. Теперь, при выпуске _latest_-версии, _prerelease_-версия будет удаляться, а все клиенты, использующие её, будут автоматически и бесшовно мигрированы. Во время этой миграции не будет никаких перебоев в работе сервиса.

    Чтобы посмотреть, какую версию вы используете сейчас, и выполнить обновление, выберите **Deploy** в верхней панели навигации и перейдите в **Environments**. Выберите нужное окружение и нажмите **Settings**. Затем нажмите **Edit**, чтобы изменить текущую версию dbt. dbt Labs рекомендует по возможности всегда использовать последнюю версию, чтобы пользоваться новыми возможностями и улучшениями.
