...
- Линтинг через SQL Fluff теперь доступен в бета-версии (GA ожидается в течение следующих 2–3 недель)
- Форматирование markdown-файлов с помощью prettier
- Использование шорткатов для улучшения developer experience, включая ``Ctrl + ` `` (переключение панели истории), `CMD + Option + /` (переключение блочного комментария), `CMD + Shift + P` (открытие палитры команд), `Option + W` (закрытие вкладки редактора)
- Отображение имени родительской папки для файлов с одинаковым названием в разделе Changes
- Быстрая навигация по новым возможностям IDE с помощью страницы справки [the IDE User Interface](/docs/cloud/studio-ide/ide-user-interface)
- Использование `top X` в SQL при предпросмотре в IDE
- Возможность подключиться к новому backend-слою IDE за последний месяц (всё ещё с использованием dbt-rpc). Бета-версия ожидается позже в июне
...
**Начиная с 1 июня 2023 года** разработчики в многоарендном приложении <Constant name="cloud" /> смогут самостоятельно получать доступ к истории запусков своего аккаунта через пользовательский интерфейс (UI) и API <Constant name="cloud" /> только за последние 365 дней, по скользящему окну. Более старая история запусков будет доступна для скачивания по запросу в службу поддержки. Мы стремимся минимизировать объём хранимых метаданных, одновременно максимизируя производительность приложения.

В частности, все `GET`‑запросы к [Runs endpoint](/dbt-cloud/api-v2#/operations/List%20Runs) dbt Cloud будут возвращать информацию о запусках, артефактах, логах и шагах запуска только за последние 365 дней. Кроме того, история запусков, отображаемая в интерфейсе dbt Cloud, будет показывать только запуски за последние 365 дней.
...
- Мы убедились, что всем известно: пользователям Cloud не нужен файл [profiles.yml](/docs/core/connect-data-platform/profiles.yml), добавив соответствующий callout на нескольких ключевых страницах.
- Дополнили страницу про [переменную Jinja model](/reference/dbt-jinja-functions/model): ранее на ней не хватало концептуальных объяснений, а также не было ссылки на страницу про schema.
- Добавили новую [landing‑страницу Quickstarts](/guides). Новый формат закладывает основу для будущих итераций, в которых появится фильтрация. А пока мы рады, что вы можете проходить quickstart‑руководства более сфокусированным образом.

    ## Проекты Cloud

- Мы запустили документ [dbt Cloud IDE user interface doc](/docs/cloud/studio-ide/ide-user-interface), который содержит подробный обзор элементов пользовательского интерфейса IDE и их назначение.
- Запустили сверкающую новую страницу [dbt Cloud Scheduler page](/docs/deploy/job-scheduler) ✨! Ранее у нас было совсем немного контента о планировщике, а теперь появился целый подраздел, в котором разобраны ключевые возможности планировщика и принципы его работы.
- Обновили страницу [dbt Cloud user license page](/docs/cloud/manage-access/seats-and-users#licenses), чтобы более чётко объяснить, как добавлять и удалять пользователей dbt Cloud.
- Опубликовали документацию по Discovery API одновременно с запуском Discovery API:
