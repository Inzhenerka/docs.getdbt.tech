...
Этот контекст можно расширить, «получив» дополнительные значения с помощью `dbt.config.get()` после того, как они будут настроены в [конфигурации модели](/reference/model-configs). Метод `dbt.config.get()` поддерживает динамический доступ к конфигурациям внутри Python‑моделей, повышая гибкость логики модели. Это включает такие входные данные, как `var`, `env_var` и `target`. Если вы хотите использовать эти значения в условной логике вашей модели, необходимо задать их через отдельную конфигурацию в YAML‑файле:
