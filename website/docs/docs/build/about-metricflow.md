...
### Расчет метрик {#calculate-metrics}
...
<DetailsToggle alt_header="Как dbt Semantic Layer обрабатывает join-ы?">
dbt <Constant name="semantic_layer" />, работающий на базе MetricFlow, строит join-ы на основе типов ключей и параметров, передаваемых в сущности (entities). Чтобы лучше понять, как формируются join-ы, ознакомьтесь с нашей документацией по типам join-ов.

Вместо описания произвольной логики соединений MetricFlow фиксирует типы каждого идентификатора и помогает пользователю выбирать корректные варианты join-ов. Это позволяет избегать fan-out и chasm join-ов, а также генерировать более понятный и читаемый SQL.
</DetailsToggle>
