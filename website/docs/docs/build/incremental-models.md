...
## Настройка инкрементальной материализации {#configure-incremental-materializations}
...
### Понимание макроса is_incremental() {#understand-the-is_incremental-macro}
...
### Фильтрация строк при инкрементальном запуске {#filtering-rows-on-an-incremental-run}
...
### Определение уникального ключа {#defining-a-unique-key}
...
#### Пример `unique_key` {#unique_key-example}
...
## Как перестроить инкрементальную модель? {#how-do-i-rebuild-an-incremental-model}
...
## Что делать, если столбцы моей инкрементальной модели изменяются? {#what-if-the-columns-of-my-incremental-model-change}
