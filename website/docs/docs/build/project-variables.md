...
dbt предоставляет механизм, [переменные](/reference/dbt-jinja-functions/var), для передачи данных моделям для
компиляции. Переменные могут использоваться для [настройки часовых поясов](https://github.com/dbt-labs/snowplow/blob/0.3.9/dbt_project.yml#L22), 
[избежания жесткого кодирования имен таблиц](https://github.com/dbt-labs/quickbooks/blob/v0.1.0/dbt_project.yml#L23)
или для передачи данных моделям для настройки их компиляции.
...
### Определение переменных в `dbt_project.yml` {#defining-variables-in-dbt_projectyml}
...
Чтобы определить переменные в проекте dbt, добавьте конфигурацию `vars` в ваш файл `dbt_project.yml`. 
Эти `vars` могут быть глобальными или относиться к конкретному пакету, 
импортированному в ваш проект.
...
### Определение переменных в командной строке {#defining-variables-on-the-command-line}

Файл `dbt_project.yml` — отличное место для определения переменных, которые редко 
меняются. Другие типы переменных, такие как диапазоны дат, будут часто изменяться. Чтобы 
определить (или переопределить) переменные для выполнения dbt, используйте опцию 
командной строки `--vars`. На практике это выглядит так:
