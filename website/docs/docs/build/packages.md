...
dbt Cloud поддерживает приватные пакеты из [поддерживаемых](#prerequisites) Git-репозиториев, используя существующую [конфигурацию](/docs/cloud/git/git-configuration-in-dbt-cloud) в вашей среде. Ранее вам нужно было настроить [токен](#git-token-method) для получения пакетов из ваших приватных репозиториев.
