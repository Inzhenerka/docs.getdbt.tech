...
- [Configure each time spine in a property file](#configuring-time-spine-in-yaml) to define how MetricFlow recognizes and uses its columns.
...
1. Add the following configuration to a new or existing property file using the [`models` key](/reference/model-properties) for the time spine in your `models/` directory. Name the property file whatever you want (for example, `util/_models.yml`):
