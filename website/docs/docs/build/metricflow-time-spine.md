...
- Определить как минимум один [time spine](#example-time-spine-tables) с той гранулярностью, которая требуется для ваших метрик (например, дневной или почасовой). При необходимости вы можете дополнительно определить таблицы с более грубой гранулярностью (например, месячной или годовой).
- [Настроить каждый time spine в YAML-файле](#configuring-time-spine-in-yaml), чтобы определить, как MetricFlow распознаёт и использует его столбцы.
...
1. Добавьте следующую конфигурацию в новый или существующий YAML-файл, используя [`models` key](/reference/model-properties) для time spine в директории `models/`. Назовите YAML-файл как угодно (например, `util/_models.yml`):
