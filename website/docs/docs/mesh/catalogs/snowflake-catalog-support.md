...
|------------------|----------|-----------------------------------------------------------------------------------------|
| `name`           | yes      | Name of catalog integration in Snowflake. For example `my_dbt_iceberg_catalog`          |
| `catalog_type`   | yes      | The type of the catalog you're building. For example, `info_schema`, `Build_in`, `_iceberg_rest`|
| `external_volume`| yes      | See [Snowflake docs](https://docs.snowflake.com/en/user-guide/tables-iceberg-configure-external-volume)|
