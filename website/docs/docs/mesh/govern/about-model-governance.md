...
- dbt Cloud builds on this with features like [cross-project ref](/docs/mesh/govern/project-dependencies) that enable collaboration at scale across multiple projects, powered by its metadata service and [dbt Explorer](/docs/explore/explore-projects). Available in dbt Cloud Enterprise or Enterprise plus plans.

All of the following features are available in dbt Core and dbt Cloud, _except_ project dependencies, which is only available to [dbt Cloud Enterprise-tier plans](https://www.getdbt.com/pricing).
