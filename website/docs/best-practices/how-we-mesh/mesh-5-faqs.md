...
## Как работает dbt Mesh {#how-dbt-mesh-works}
...
<DetailsToggle alt_header="Можно ли применять model contracts к source-данным?">

Нет, contracts можно применять только на [уровне модели](/docs/mesh/govern/model-contracts). В качестве рекомендуемой best practice стоит [определять staging models](/best-practices/how-we-structure/2-staging) поверх sources, и уже на эти staging-модели можно задавать contracts.

</DetailsToggle>

<DetailsToggle alt_header="Можно ли применять contracts частично?">
...
<DetailsToggle alt_header="Можно ли указать нескольких владельцев в группе?">

Нет, [группу](/docs/mesh/govern/model-access#groups) можно назначить только одному владельцу. Однако назначенным владельцем может быть _команда_, а не отдельный человек.
...
## Разрешения и доступ {#permissions-and-access}
...
## Совместимость с другими возможностями {#compatibility-with-other-features}
...
## Советы по внедрению dbt Mesh {#tips-on-implementing-dbt-mesh}
