...
## Какую проблему решает этот метод? {#what-pain-is-being-solved}
...
## Чему вы научитесь после прочтения этой статьи? {#what-will-you-learn-after-reading-this-article}
...
## Генерация `.yml` с помощью пакета Codegen {#generate-yml-with-codegen-package}
...
### Генерация `.yml` для нескольких моделей сразу {#generate-yml-for-several-models-at-once}
...
### Получение имен моделей программно {#get-model-names-programmatically}
...
## Создание блоков docs для новых колонок {#create-docs-blocks-for-the-new-columns}
...
### Получение всех уникальных колонок в папке {#get-all-unique-columns-within-the-folder}
...
### Форматирование для соответствия с Jinja блоками docs {#format-to-align-with-jinja-docs-block}
...
## Обновление `.yml` файла для получения документации из `.md` файла {#update-yml-file-to-source-documentation-from-the-md-file}
...
## Проверьте, что все работает {#check-that-everything-works}
