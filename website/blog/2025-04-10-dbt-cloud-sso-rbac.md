...
As a dbt Cloud admin, you’ve just upgraded to dbt Cloud on the Enterprise plan - **congrats**! dbt Cloud has a lot to offer such as CI/CD, Orchestration, Explorer, Semantic Layer, dbt Mesh, Visual Editor, dbt Copilot, and so much more. ***But where should you begin?*** We strongly recommend as you start adopting dbt Cloud functionality to make it a priority to set up single sign-on (SSO) and role-based access control (RBAC). This foundational step enables your organization to keep your data pipelines secure, onboard users into dbt Cloud with ease, and optimize cost savings for the long term.

# Authentication vs. Authorization

Before we dig into SSO & RBAC more, let’s go over how they map into two foundational security concepts.
...
Regardless of what IdP you use, one of the first things you should do as a dbt Cloud admin is set the **login slug** value. This should be a *unique company identifier*. Keep in mind that whatever you set for it will be appended to the end of the SSO login URL your users may leverage to sign into dbt Cloud. For example, if I set my login slug to mynewco, my SSO login URL will look something like https://cloud.getdbt.com/enterprise-login/mynewco. 

Even with our documentation at hand, staring at this screen as a dbt Cloud admin can be daunting.
...
1. After setting the desired login slug, a *dbt Cloud admin* will go to the SSO configuration page and copy/paste everything under **Identity provider values** and will pass them along to the *IdP admin*.
2. The *IdP admin* will create the dbt Cloud app following the documentation above and then provide the values under **dbt configuration** to the *dbt Cloud admin*.
3. The *dbt Cloud admin* will plug in those values into the SSO configuration page under **dbt configuration** and click ‘Save’ to complete the process.

After that process has been completed, we strongly advise you validate the SSO flow is working by pasting the SSO login URL (it should look like https://cloud.getdbt.com/enterprise-login/dbtlabs) into your web browser’s private window and trying to log into your account via the IdP. If the SSO flow isn’t working as expected, an account admin will still be able to log in with a password to correct the configuration.

Be aware of our [SSO enforcement policy](https://docs.getdbt.com/docs/cloud/manage-access/sso-overview#sso-enforcement); TL;DR - once SSO is configured all non-admin users will have to log in via SSO as a security best practice while account admins by default can still authenticate with a password in lieu of multi-factor authentication (MFA).

Once SSO has been completed successfully, you have to additional ways to onboard your users into dbt Cloud on top of sending out an email invite -
...
In dbt Cloud, there are two main levers to control user access - licenses and groups. As a prerequisite, these all should be set before configuring RBAC. Let’s get into these!
...
<Lightbox src="/img/blog/2025-04-10-sso-and-rbac/3_license_mapping_example" title="An example license mapping" width="85%" />
...
Navigating to Groups & Licenses, out of the box you’ll see three default groups - Everyone, Member, and Owner. There’s also an option to create your own groups on the top right.
...
Additionally to setting permission sets, in a group you may set which projects and environments the users should have access to.
