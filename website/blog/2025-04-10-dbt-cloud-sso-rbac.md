...
As a dbt Cloud admin, you’ve just upgraded to dbt Cloud on the [Enterprise plan](https://www.getdbt.com/pricing) - **congrats**! dbt Cloud has a lot to offer such as CI/CD, Orchestration, Explorer, Semantic Layer, dbt Mesh, Visual Editor, dbt Copilot, and so much more. ***But where should you begin?*** 
...
- **Authentication:** [SSO](/blog/dbt-cloud-sso-rbac#single-sign-on) is configured to gate authentication - it verifies (via an IdP) that users are who they say they are and can log into the specified dbt Cloud account.
