...
## Где Airflow + dbt согласуются {#where-airflow-dbt-align}
...
### Со стороны Airflow {#from-the-airflow-side}
...
### Со стороны dbt {#from-the-dbt-side}
...
## Заметка о ролях команд данных для Airflow + dbt {#a-note-on-data-team-roles-for-airflow-dbt}
...
## Как Airflow + dbt хорошо работают вместе {#how-airflow-dbt-play-nicely}
...
### Наблюдаемость конвейера для аналитиков {#pipeline-observability-for-analysts}
...
### Наблюдаемость трансформаций для инженеров {#transformation-observability-for-engineers}
...
## Почему я ❤️ dbt Cloud + Airflow {#why-i-dbt-cloud-airflow}
...
### dbt Core и Airflow {#dbt-core-airflow}
...
#### Повторный запуск задач с точки сбоя {#rerunning-jobs-from-failure}
...
### dbt Cloud и Airflow {#dbt-cloud-airflow}

#### Использование провайдера dbt Cloud {#using-the-dbt-cloud-provider}
...
#### Настройка Airflow и dbt Cloud {#setting-up-airflow-and-dbt-cloud}
