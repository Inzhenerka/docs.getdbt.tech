...
## Почему люди используют cherry pick для верхних веток? {#why-do-people-cherry-pick-into-upper-branches}
...
## Если вы не тестируете изменения независимо, не стоит продвигать их независимо {#if-youre-not-testing-changes-independently-you-shouldnt-promote-them-independently}
...
## Ветки для функций содержат больше, чем кажется на первый взгляд {#feature-branches-contain-more-than-meets-the-eye}
...
## Повторяющиеся конфликты слияния отнимают время разработки {#repeated-merge-conflicts-drain-development-time}
...
## Что делать вместо этого: Рекомендуемая стратегия ветвления для многосредовых проектов dbt {#what-to-do-instead-the-recommended-branching-strategy-for-multi-environment-dbt-projects}
