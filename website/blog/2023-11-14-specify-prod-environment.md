...
- Создавая ваши производственные модели в [другой схеме и базе данных](https://docs.getdbt.tech/docs/build/custom-schemas#managing-environments), вы можете экспериментировать спокойно, не беспокоясь о том, что ваши изменения случайно повлияют на пользователей ниже по потоку.
...
## Убедитесь, что dbt Explorer всегда имеет самую свежую информацию {#make-sure-dbt-explorer-always-has-the-freshest-information-available}
...
## Запускайте более компактный CI с отложением на уровне среды {#run-slimmer-ci-than-ever-with-environment-level-deferral}
