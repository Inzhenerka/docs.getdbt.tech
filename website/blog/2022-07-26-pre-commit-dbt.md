...
## Что такое pre-commit и pre-commit-dbt? {#what-are-pre-commit-and-pre-commit-dbt}
...
## Внедрение pre-commit-dbt и добавление тестов {#implementing-pre-commit-dbt-adding-tests}
...
### 1. Определите наши правила и создайте файл конфигурации pre-commit {#1-define-our-rules-and-create-a-pre-commit-config-file}
...
### 2. Понять объем необходимых изменений {#2-understand-the-scope-of-the-changes-required}

С созданным конфигурационным файлом мы теперь можем настроить наш компьютер для выполнения некоторых проверок локально.

#### Активация виртуальной среды Python {#activating-a-python-virtual-environment}
...
#### Установка и запуск pre-commit {#installing-and-running-pre-commit}
...
### 3. Решите, что нужно исправить немедленно {#3-decide-what-needs-to-be-fixed-immediately}
...
### 4. Сделайте это частью периодических проверок {#4-make-it-part-of-the-periodic-checks}
...
#### a) Добавление pre-commit-dbt в поток CI (работает для пользователей dbt Cloud и dbt Core) {#a-adding-pre-commit-dbt-to-the-ci-flow-works-for-dbt-cloud-and-dbt-core-users}
...
#### b) Установка хуков git pre-commit (для пользователей dbt Core) {#b-installing-the-pre-commit-git-hooks-for-dbt-core-users}
...
#### c) Итак, тесты в CI или хуки git локально? {#c-so-tests-in-ci-or-git-hooks-locally}
...
## Развитие этого решения {#taking-this-solution-further}
