...
Самая важная команда — это [`dbt run`](https://docs.getdbt.tech/reference/commands/run). Но в развертывании мы редко используем только `dbt run`.

Вы, вероятно, уже знакомы с множеством [команд dbt](https://docs.getdbt.tech/reference/dbt-commands), и этот раздел посвящен тому, чтобы показать вам, какие команды, по нашему мнению, должны быть наиболее эффективными при запуске вашего проекта dbt в производственной среде!
...
## Принципы команд развертывания dbt {#principles-of-dbt-deployment-commands}

<WistiaVideo id="7em2tj62yo" />

### 1) Всегда тестируйте ваши данные {#1-always-test-your-data}
...
### 2) Понимание влияния задержанных данных в вашем проекте {#2-understand-the-impact-of-delayed-data-in-your-project}

Некоторые проекты могут позволить себе иметь более старые данные в хранилище, другие — нет.


### 3) Упрощение регулярных запланированных запусков {#3-simplify-your-regularly-scheduled-runs}
...
## Самые простые возможные производственные команды dbt {#simplest-possible-production-dbt-commands}
...
## Предпочтительные производственные команды dbt {#preferred-production-dbt-commands}
...
## Чем отличаются ваши команды запуска? {#how-do-your-run-commands-differ}
