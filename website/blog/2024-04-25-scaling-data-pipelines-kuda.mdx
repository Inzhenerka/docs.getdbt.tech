| title | description | slug | authors | tags | hide_table_of_contents | date | is_featured 
| --- | --- | --- | --- | --- | --- | --- | --- |
| Scaling Data Pipelines for a Growth-Stage Fintech with Incremental Models | How Kuda leveraged dbt incremental models to reduce costs, speed up pipelines, and scale confidently. | scaling-data-pipelines-fintech | `adedamola_onabanjo` | `analytics` `dbt Cloud` `fintech` `BigQuery` | false | 2025-04-23 | true |
...
In dbt, using append is straightforward. We configure the model as incremental and specify `incremental_strategy='append'` (for BigQuery and similar warehouses). In the SQL, we filter the source to only new records since the last load. For example, to incrementally load new transactions:
