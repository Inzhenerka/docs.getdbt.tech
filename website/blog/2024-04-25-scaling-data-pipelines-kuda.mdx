...
In dbt, using append is straightforward. We configure the model as incremental and specify `incremental_strategy='append'` (for [BigQuery](/reference/resource-configs/bigquery-configs#merge-behavior-incremental-models) and [similar warehouses](/docs/build/incremental-strategy#supported-incremental-strategies-by-adapter)). In the SQL, we filter the source to only new records since the last load. For example, to incrementally load new transactions:
