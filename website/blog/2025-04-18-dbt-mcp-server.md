...
# Представляем dbt MCP Server — перенос структурированных данных в AI‑воркфлоу и агентные системы {#introducing-the-dbt-mcp-server-bringing-structured-data-to-ai-workflows-and-agents}
...
## **Что такое MCP?** {#using-the-dbt-mcp-server-for-data-asset-discovery}
...
## **Что делает dbt MCP Server?** {#using-the-dbt-mcp-server-for-querying-data-via-the-dbt-semantic-layer}
...
## **Использование dbt MCP Server для обнаружения data‑assets** {#using-the-dbt-mcp-server-for-sql-execution-to-power-text-to-sql}
...
### **Инструменты для обнаружения data‑assets:** {#project-execution-tools}
...
## **Использование dbt MCP server для запросов через dbt Semantic Layer** {#getting-started}
...
## **Использование dbt MCP server для выполнения SQL (text‑to‑SQL)** {#what-is-the-best-workflow-for-the-current-iteration-of-the-mcp-server}
...
## **Использование dbt MCP server для выполнения проекта** {#the-future-of-the-dbt-mcp-and-the-correct-layers-of-abstraction-for-interfacing-with-your-data}
...
Инструменты используют два основных источника информации — dbt Cloud APIs и dbt CLI. Мы продолжим развивать оба направления, особенно dbt Cloud APIs как предпочтительный уровень абстракции при работе с конкретным [environment](https://docs.getdbt.tech/docs/dbt-cloud-environments).
