...
## История качества данных в dbt {#story-of-data-quality-in-dbt}

Основная причина моих плохих снов — это беспокойство о некачественных данных, которые влияют на общие результаты.

Одной из вещей, которая мне сразу понравилась, когда я начал использовать dbt, было то, что он имел полноценный механизм для утверждения качества данных на всех наших производственных данных в виде [тестов данных](https://docs.getdbt.tech/docs/build/data-tests).
...
## Введение модульного тестирования в dbt {#introducing-unit-testing-in-dbt}
...
## Привет, мир модульного тестирования {#hello-unit-testing-world}
...
## Создание модульных тестов {#crafting-unit-tests}
...
### Организация ваших мыслей {#organizing-your-thoughts}
...
### Написание ваших модульных тестов {#writing-your-unit-tests}
...
## Пример из реального мира {#real-world-example}
...
### YAML модульного теста {#unit-test-yaml}
...
### Оригинальный SQL код {#original-sql-code}
...
### Исправление SQL кода {#sql-code-fix}
...
## Модульные тесты vs. контракты моделей vs. тесты данных {#unit-tests-vs-model-contracts-vs-data-tests}

dbt предоставляет несколько взаимодополняющих возможностей для обеспечения качества данных, включая [unit tests](https://docs.getdbt.tech/docs/build/unit-tests), [model contracts](https://docs.getdbt.tech/docs/mesh/govern/model-contracts) и [data tests](https://docs.getdbt.tech/docs/build/data-tests). Ниже приведена таблица с их сравнением и пояснением, в каких случаях стоит использовать каждый из подходов:
...
Более подробную информацию о синтаксисе вы можете найти в нашей [документации](https://docs.getdbt.tech/docs/build/unit-tests). Надеемся, это даст вам инструменты для повышения уверенности в ваших конвейерах данных и позволит спать спокойнее ночью 😴